...
use Modules\WebBill\Models\Timekeeper;
use Validator;
...
class TimekeeperController extends CURDBaseController
{
    ...
    protected $module = [
        ...
        'modal' => '\Modules\WebBill\Models\Timekeeper',
        'list' => [
        	['name' => 'admin_id', 'type' => 'relation', 'label' => 'Họ tên', 'object' => 'admin', 'display_field' => 'name'],
            ...
            ['name' => 'thoi_gian_muon', 'type' => 'custom', 'td' => 'webbill::timekeeper.list.td.thoi_gian_muon', 'label' => 'Thời gian muộn'],
        ],
        'form' => [
            'general_tab' => [
            	['name' => 'admin_id', 'type' => 'select2_ajax_model', 'label' => 'Thành viên', 'model' => Admin::class, 'object' => 'admin', 'display_field' => 'name', 'group_class' => 'col-md-6'],
                ['name' => 'may_cham_cong_id', 'type' => 'number', 'class' => '', 'label' => 'ID máy chấm công', 'group_class' => 'col-md-6'],
                ['name' => 'time', 'type' => 'datetime-local', 'class' => '', 'label' => 'Thời gian chấm', 'group_class' => 'col-md-6'],
                ['name' => 'thoi_gian_muon', 'type' => 'number', 'class' => '', 'label' => 'Thời gian muộn', 'group_class' => 'col-md-6'],        
            ],
    ...
    public function getIndex(Request $request)
    {
        ...
        return view('webbill::timekeeper.list')->with($data);
    }

    public function appendWhere($query, $request)
    {
        //  Nếu không có quyền xem toàn bộ dữ liệu thì chỉ được xem dữ liệu cá nhân mình
        if (!CommonHelper::has_permission(\Auth::guard('admin')->user()->id, 'timekeeper')) {
    ...
    public function add(Request $request)
    {
        try {
            ...
            if (!$_POST) {
                ...
                return view('webbill::timekeeper.add')->with($data);
            } else if ($_POST) {
                \DB::beginTransaction();
    ...
    public function update(Request $request)
    {
        try {
            ...
            if (!$_POST) {
                ...
                return view('webbill::timekeeper.edit')->with($data);
            } else if ($_POST) {
                \DB::beginTransaction();
...
//            CommonHelper::one_time_message('error', 'Lỗi hệ thống! Vui lòng liên hệ kỹ thuật viên.');
    ...
    public function importItem($row, $r, $dataInsertFix)
    {
        try {
            ...
            if (!$row_empty) {
                echo '__bắt đầu insert:' . $row->all()['may_cham_cong_id'];
                $data = [];
