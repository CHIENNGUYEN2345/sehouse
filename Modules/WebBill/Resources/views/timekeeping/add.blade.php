...
$timekeeping = \Modules\WebBill\Models\Timekeeping::where('admin_id', \Auth::guard('admin')->user()->id)->where('day', $day)->first();
?>
                    ...
                    <div class="kt-form">
                        <div class="kt-portlet__body">
                            <div class="kt-section kt-section--first contacted_log_list">
                                ...
                                $logs = [];
                                $lead_contacted_logs = \Modules\WebBill\Models\LeadContactedLog::select('lead_id', 'note', 'created_at', 'type')->where('admin_id', $saler_id)
                                ...
                                $leads = \Modules\WebBill\Models\Lead::select('id', 'tel', 'name', 'created_at')->where('admin_id', \Auth::guard('admin')->user()->id)
                                    ->where('created_at', '>=', date('Y-m-d 00:00:00', strtotime($day)))
                                    ->where('created_at', '<=', date('Y-m-d 23:59:59', strtotime($day)))->orderBy('id', 'asc')->get();
                                foreach($leads as $v) {
                                ...
                                $coun_lead_contacted_logs = \Modules\WebBill\Models\LeadContactedLog::where('admin_id', $saler_id)
