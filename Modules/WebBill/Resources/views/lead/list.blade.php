...
<div class="kt-container  kt-container--fluid  kt-grid__item kt-grid__item--fluid">
    <?php
    $count_leads_created = \Modules\WebBill\Models\Lead::whereRaw($whereSale)
    ->whereRaw($whereCreated_at)
    ->count();

    $coun_lead_contacted_logs = \Modules\WebBill\Models\LeadContactedLog::whereRaw($whereSale)
    ->whereRaw($whereCreated_at)
    ->whereNotIn('note', ['Không nghe', 'Sai số'])->count();

    $count_khqt = \Modules\WebBill\Models\Lead::whereRaw($whereLikeSale)
    ...
    $quanTamMoi = \Modules\WebBill\Models\LeadContactedLog::whereRaw($whereSale)
    ->where('created_at', '>', date('Y-m-d 00:00:00'))
    ->where('type', 'lead_quan_tam_lai')->count();

    $count_khqt_cao = \Modules\WebBill\Models\Lead::whereRaw($whereLikeSale)
    ->whereIn('rate', ['Quan tâm cao'])->whereNotIn('status', ['Thả nổi', 'Đã ký HĐ'])
                // ->where('created_at', '>=', date('Y-'.date('m', strtotime($day)).'-01 00:00:00'))
                // ->where('created_at', '<=', date('Y-'.date('m', strtotime($day)).'-t 23:59:59', strtotime($day)))
    ->count();
    $count_co_hoi = \Modules\WebBill\Models\Lead::whereRaw($whereLikeSale)
    ...
    $plan = \Modules\WebBill\Models\Plan::whereRaw($whereSale)->orderBy('id', 'desc')->first();
    $ds_tuan = \Modules\WebBill\Models\Bill::whereRaw($whereSaler)
    ->where('registration_date', '>=', date('Y-m-d 00:00:00', strtotime('-'.date('w').' days')))
    ->sum('total_price');
    $ds_thang = \Modules\WebBill\Models\Bill::whereRaw($whereSaler)
    ->where('registration_date', '>=', date('Y-m-01 00:00:00', time()))
    ->sum('total_price');
    $hd = \Modules\WebBill\Models\Bill::whereRaw($whereSaler)
    ->where('registration_date', '>=', date('Y-m-d 00:00:00', strtotime('-'.date('w').' days')))
    ->count();
    ...
    <div class="lead-chart">
        <label style="color: red;">Mục tiêu tuần @if(is_object($plan))({{ date('d/m', strtotime(@$plan->updated_at)) }})@endif</label>
        <span>Tạo mới: {{ $count_leads_created }}/60.</span>&nbsp;&nbsp;&nbsp;&nbsp;
        <span><a target="_blank" href="/admin/timekeeping/add">Tương tác: {{ $coun_lead_contacted_logs }}/60.</a></span>&nbsp;&nbsp;&nbsp;&nbsp;
        ...
        <span title="Doanh số thực / mục tiêu tuần">DS: {{ number_format($ds_tuan/1000000, 1, ',', '.') }}/@if(is_object($plan)){{ number_format(@$plan->ds_tuan, 0, '.', '.') }}@endif.</span>&nbsp;&nbsp;&nbsp;&nbsp;
        <div style="display: inline-block; float: right;">
            <label style="color: red;">Mục tiêu tháng</label>
            <span title="Doanh số thực / mục tiêu tháng">DS: {{ number_format($ds_thang/1000000, 1, ',', '.') }}/@if(is_object($plan)){{ number_format(@$plan->ds_thang, 0, '.', '.') }}@endif.</span>
        </div>
    </div>
...
<!--end: Datatable -->
    ...
    @if(in_array($module['code'] . '_assign', $permissions))
        ...
        @include('webbill::lead.list.partials.lead_assign')
    @endif

    @if(in_array(CommonHelper::getRoleName(\Auth::guard('admin')->user()->id, 'name'), ['telesale', 'sale', 'truong_phong_sale']))
        @include('webbill::lead.list.partials.nhac_nho_telesale')
    @endif
@endsection
