                    ...
                    <div class="kt-form">
                        <div class="kt-portlet__body">
                            ...
                            @if(in_array('lead_assign', $permissions))
                                <?php
                                    $field = ['name' => 'saler_ids', 'type' => 'custom', 'field' => 'webbill::lead.form.saler_ids', 'label' => 'Sale phụ trách', 'model' => \App\Models\Admin::class, 'object' => 'admin', 'display_field' => 'name', 'multiple' => true, 'group_class' => 'col-md-12'];
                                    $field['value'] = @$result->{$field['name']};
                                    ?>
                                    <div class="form-group-div form-group {{ @$field['group_class'] }}"
                                             id="form-group-{{ $field['name'] }}">
                                            <label for="{{ $field['name'] }}">{{ @$field['label'] }} @if(strpos(@$field['class'], 'require') !== false)
                                                    <span class="color_btd">*</span>@endif</label>
                                            <div class="col-xs-12">
                                    @include('webbill::lead.form.saler_ids', ['field' => $field])
                                            </div>
                                        </div>
                            @endif
...
<p style="font-size: 13px; margin: 0;">Chuyển trạng thái: thả nối -> đang chăm sóc</p>
</div>
...
<i></i>
<div class="log-content">
<p style="font-size: 13px; margin: 0;">Chuyển đánh giá: Tìm hiểu -> Quan tâm cao</p>
</div>
...
<div class="kt-portlet__head-wrapper">
 <div class="">
<input type="text" name="quick_search" value="" class="form-control" title="Chỉ cần enter để thực hiện tìm kiếm" placeholder="Tìm kiếm nhanh theo ID, giá trị">
</div>
...
<li class="kt-nav__item">
<a class="kt-nav__link export-excel" title="Xuất các bản ghi đang lọc ra file excel" onclick="$('input[name=export]').click();">
...
&nbsp;
<a href="https://hbsoft.top/admin/bill_histories/add?bill_id=15" class="btn btn-brand btn-elevate btn-icon-sm">
...
<form class="kt-form kt-form--fit kt-margin-b-20 form-search" id="form-search" method="GET" action="" style="display: none;">
<input name="search" type="hidden" value="true">
<input name="limit" type="hidden" value="20"><input type="hidden" name="quick_search" value="" id="quick_search_hidden" class="form-control" placeholder="Tìm kiếm nhanh theo ID, giá trị"><input type="hidden" name="quick_search" value="" id="quick_search_hidden" class="form-control" placeholder="Tìm kiếm nhanh theo ID, giá trị">
<div class="row">
<div class="col-sm-6 col-lg-3 kt-margin-b-10-tablet-and-mobile list-filter-item">
<label>:</label>
<input type="text" name="bill_id" placeholder="" value="15" class="form-control kt-input hidden"> </div>
</div>
<div class="row">
...
&nbsp;&nbsp;
<a class="btn btn-secondary btn-secondary--icon" id="kt_reset" title="Xóa bỏ bộ lọc" href="/admin/bill_histories?bill_id=15">
<span>
...
<th data-field="expiry_date" class="kt-datatable__cell kt-datatable__cell--sort " onclick="sort('expiry_date')">
Nội dung
<i class="flaticon2-arrow-down"></i>
</th>
<th data-field="price" class="kt-datatable__cell kt-datatable__cell--sort " onclick="sort('price')">
Hành động
<i class="flaticon2-arrow-down"></i>
</th>
<th data-field="note" class="kt-datatable__cell kt-datatable__cell--sort " onclick="sort('note')">
...
<td class="kt-datatable__cell--center kt-datatable__cell kt-datatable__cell--check" data-field="ID"><span style="width: 20px;"><label class="kt-checkbox kt-checkbox--single kt-checkbox--solid"><input name="id[]" type="checkbox" class="ids" value="14">&nbsp;<span></span></label></span>
</td>
<td data-field="expiry_date" class="kt-datatable__cell item-expiry_date">
Đơn này được sửa chưa?<br>09:29 10/04/2022 - Bởi: Đỗ Xuân Bách </td>
...
<div class="dropdown bootstrap-select kt-datatable__pager-size select-page-size" style="width: 60px;"><select class="selectpicker kt-datatable__pager-size select-page-size" onchange="$('input[name=limit]').val($(this).val());$('#form-search').submit();" title="Chọn số bản ghi hiển thị" data-width="60px" data-selected="20" tabindex="-98"><option class="bs-title-option" value=""></option>
<option value="20" selected="">20</option>
<option value="30">30</option>
<option value="50">50</option>
<option value="100">100</option>
</select><button type="button" class="btn dropdown-toggle btn-light" data-toggle="dropdown" role="combobox" aria-owns="bs-select-1" aria-haspopup="listbox" aria-expanded="false" title="20"><div class="filter-option"><div class="filter-option-inner"><div class="filter-option-inner-inner">20</div></div> </div></button><div class="dropdown-menu "><div class="inner show" role="listbox" id="bs-select-1" tabindex="-1"><ul class="dropdown-menu inner show" role="presentation"></ul></div></div></div>
</div>
