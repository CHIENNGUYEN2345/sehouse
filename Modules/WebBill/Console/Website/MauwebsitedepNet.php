...
class MauwebsitedepNet extends CrawlProductBase
{
...
//    protected $cat_link = 'https://webrt.vn/danhmuc/ban-dien-lanh-dien-may-dien-dan-dung/';
    ...
    protected $cat_name = 'Bán hàng';
    protected $cat_link = 'http://mauwebsitedep.net/mau-website-dep/';
...
//    protected $cat_link = 'https://webrt.vn/danhmuc/web-nha-thuoc-phong-kham/';
    ...
    public function crawlPageList($test = false)
    {
        ...
        while (!$stop) {
            ...
            $products_find = $html->find('.section-content .col-md-4.col-sm-6.col-xs-12.col');
            ...
            foreach ($products_find as $k => $product) {
                ...
                try {
                    ...
                    //  Lấy link sản phẩm
                    $product_link = $product->find('a.demo', 0);

                    if ($product_link == null) {
                        return false;
                    }

                    $html2 = file_get_html($product_link->getAttribute('href'));
...
//                            dd($product_data);
                                ...
                                //  Chưa thì tạo mới
                                if ($product->find('.theme .thumbnail img', 0) != null) {
                                    $product_data['image'] = $product->find('.theme .thumbnail img', 0)->getAttribute('data-lazy-src');
                                    if ($product_data['image'] == null) {
                                        $product_data['image'] == $product->find('.theme .thumbnail img', 0)->getAttribute('src');
                                    }
                                }
                                if ($product->find('.theme .theme-title', 0) != null) {
                                    $product_data['name'] = $product->find('.theme .theme-title', 0)->innertext;
