...
class CrawlProductBase extends Base
{
    ...
    protected $module = [
        ...
        'table_name' => 'products',
        'label' => 'Sản phẩm',
        'modal' => '\Modules\WebBill\Models\Codes',
    ];
...
//        $this->_doom_setting = json_decode($website->doom);
    ...
    public function runUpdateProduct($test, $whereRaw) {
        $result = [
            'total_created' => 0,
            'total_updated' => 0
        ];

        $products = Product::whereRaw($whereRaw)->get();

        //  Thực hiện quét các danh mục đã cấu hình
        foreach ($products as $k => $product_exist) {

            try {

                //  Đã có thì  cập nhật - chưa thì tạo mới
                $product_data = $this->getDataProduct($product_exist->crawl_link);
                $product_data = $this->cleanData($product_data);
                $product_data = $this->appendData($product_data, null);
                if ($test) {
                    return $this->printDemo($product_data);
                }
                $this->updateProduct($product_exist, null, $product_data);
...
//                        $product_link = 'https://timnhaphanphoi.vn/cha-ca-han-quoc-ds15952.html';
                        if ($product_link != $link_old) {
                            ...
                            if (is_object($product_exist)) {
                                ...
                                $product_data = $this->cleanData($product_data);
                                $product_data = $this->appendData($product_data, $cat_doom);
                                if ($test) {
                                    return $this->printDemo($product_data);
                                }
                                $this->updateProduct($product_exist, $cat_doom, $product_data);
                                $result['total_updated'] ++ ;
                            } else {
                                //  Chưa thì tạo mới
                                $product_data = $this->getDataProduct($product_link);
...
//        $product->status = 1;
    ...
    public function getContent($data, $html)
    {
        ...
        if ($v != null) {
            ...
            $data['content'] = $this->saveImgInContent($data['content'], $v, 'product/' . date('Y/m/d/') . str_slug($data['name']) . '/content');
    ...
    public function getImage($data, $html)
    {
        ...
        if ($v != null) {
            ...
            if ($data['image'] != '') {
                ...
                try {
                    $data['image'] = CommonHelper::saveFile($data['image'], 'product/' . date('Y/m/d/') . str_slug($data['name']));
                } catch (\Exception $ex) {
        ...
        if ($v != null) {
            ...
            foreach ($v as $image_extra) {
                ...
                if ($image_extra_src != '') {
                    ...
                    try {
                        $image_extra_arr[] = CommonHelper::saveFile($image_extra_src, 'product/' . date('Y/m/d/') . str_slug($data['name']));
                    } catch (\Exception $ex) {
