...
//    protected $cat_link = 'https://webrt.vn/danhmuc/ban-dien-lanh-dien-may-dien-dan-dung/';
    ...
    protected $cat_name = 'Bán hàng';
    protected $cat_link = 'https://thaibinhweb.net/shop/';
...
//    protected $cat_link = 'https://webrt.vn/danhmuc/web-nha-thuoc-phong-kham/';
    ...
    public function crawlPageList($test = false)
    {
        ...
        while (!$stop) {
            ...
            $products_find = $html->find('.products .product');
...
//                            dd($product_data);
                                ...
                                //  Chưa thì tạo mới
                                if ($product->find('.box-image img', 0) != null) {
                                    $product_data['image'] = $product->find('.box-image img', 0)->getAttribute('data-src');
                                    if ($product_data['image'] == null) {
                                        $product_data['image'] = $product->find('.box-image img', 0)->getAttribute('src');
                                    }
                                    $product_data['image'] = str_replace('-330x330', '', $product_data['image']);
                                }


                                if ($product->find('.product-title a', 0) != null) {
                                    $product_data['name'] = $product->find('.product-title a', 0)->innertext;
