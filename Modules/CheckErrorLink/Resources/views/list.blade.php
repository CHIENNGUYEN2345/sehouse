                ...
                <div class="kt-portlet__head-toolbar">
                    <div class="kt-portlet__head-wrapper">
                        <div class="">
                            ...
                            <div class="dropdown dropdown-inline">
                                ...
                                <div class="dropdown-menu dropdown-menu-right" x-placement="bottom-end"
                                    ...
                                    <ul class="kt-nav">
                                        ...
                                        {{--                                        @if(in_array($module['code'] . '_delete', $permissions))--}}
                                        <li class="kt-nav__item">
                                            <a class="kt-nav__link" onclick="allDelete();">
                            ...
                            <a @if(is_null(@$result))
                               href="{{ url('/admin/'. $module['code'].'/add') }}"
                               @else
                               href="{{ url('/admin/'. $module['code'].'/add/'.@$result->id) }}"
                               @endif
                            ...
                            {{--                            @if(in_array('super_admin', $permissions))--}}
                            <a @if(is_null(@$result))
                               href="{{ url('/admin/'. $module['code'].'/run') }}"
                               @else
                               href="{{ url('/admin/'. $module['code'].'/run/'.@$result->id) }}"
                               @endif
            ...
            <div class="kt-portlet__body kt-portlet__body--fit">
                ...
                <div class="kt-datatable kt-datatable--default kt-datatable--brand kt-datatable--scroll kt-datatable--loaded"
                    ...
                    <table class="table table-striped">
                        ...
                        @if(isset($module['group_by']))
                            @foreach($listItem as $id=>$group)
                                @foreach($group as $number => $item)
                                    {{--{{dd($item )}}--}}
                                    <tr data-row="0" class="kt-datatable__row" style="left: 0px;">
                                        ...
                                        @foreach($module['list'] as $field)
                                            @if($number > 0 && @$module['group_by'] == $field['name'] )

                                            @else
                                                <td @if($number == 0 && @$module['group_by'] == $field['name'] )
...
@section('custom_head')
    ...
    {{--    <link type="text/css" rel="stylesheet" charset="UTF-8" href="{{ asset('Modules\EworkingAdmin\Resources\assets\css\custom.css') }}">--}}
@endsection
@section('custom_footer')
    <script src="{{ asset(config('core.admin_asset').'/js/pages/crud/metronic-datatable/advanced/vertical.js') }}"
            type="text/javascript"></script>
    <script src="{{ asset(config('core.admin_asset').'/js/list.js') }}"></script>
    {{--    <script type="text/javascript" src="{{ asset('Modules\EworkingAdmin\Resources\assets\js\custom.js') }}"></script>--}}
    @include(config('core.admin_theme').'.partials.js_common')
    <script>
        function allDelete() {
            if (confirm('Bạn có chắc chắn muốn xóa?')) {
                $.ajax({
                    url: '/admin/check_error_link/all-delete',
                    type: 'GET',
                    data: {},
