...
class DHBillController extends CURDBaseController
{
    ...
    protected $module = [
        ...
        'list' => [
            ...
            ['name' => 'customer_id', 'type' => 'relation_edit', 'label' => 'Học viên', 'object' => 'admin', 'display_field' => 'name'],
            ['name' => 'customer_id', 'type' => 'relation', 'label' => 'SĐT', 'object' => 'admin', 'display_field' => 'tel'],
            
            ['name' => 'service_id', 'type' => 'custom', 'label' => 'Lớp học', 'td' => 'CRMEdu.dhbill.list.td.service'],
            ['name' => 'progress_id', 'type' => 'relation', 'label' => 'Tiến độ', 'object' => 'bill_progress', 'display_field' => 'status'],
            ['name' => 'finance_id', 'type' => 'custom', 'td' => 'CRMEdu.dhbill.list.td.thanh_toan', 'label' => 'Thanh toán', 'object' => 'bill_finance', 'display_field' => 'debt'],
            ['name' => 'saler_id', 'type' => 'relation', 'label' => 'Kinh doanh', 'object' => 'saler', 'display_field' => 'name'],
            ['name' => 'progress_id', 'type' => 'relation_through', 'label' => 'Trợ giảng', 'object' => 'bill_progress', 'object2' => 'dieu_hanh', 'display_field' => 'name'],
            ['name' => 'progress_id', 'type' => 'relation_through', 'label' => 'Giảng viên', 'object' => 'bill_progress', 'object2' => 'ky_thuat', 'display_field' => 'name'],
        ...
        'form' => [
            ...
            'customer_tab' => [
                ...
                ['name' => 'customer_id', 'type' => 'custom', 'type_history' => 'relation_multiple', 'field' => 'CRMEdu.form.fields.select_customer',
                    'label' => 'học viên', 'model' => Admin::class, 'object' => 'admin', 'display_field' => 'name', 'display_field2' => 'tel', 'class' => 'required', 'inner' => 'disabled'],
        ...
        foreach($bills as $bill) {
            ...
            if(is_object($bill_process)) {
                if ($bill_process->dh_id == $ky_thuat_id) {
                    //  nếu dự án do mình Trợ giảng thì tính điểm Trợ giảng
                ...
                if ($bill_process->kt_id == $ky_thuat_id) {
                    //  nếu dự án do mình triển khai thì tính điểm Giảng viên
    ...
    public function appendWhere($query, $request)
    {
        ...
        //  Lọc theo người Trợ giảng / Giảng viên
        if (@$request->ky_thuat_id != null) {
            $bill_ids = BillProgress::where(function ($query) use ($request) {
                            $query->orWhere('dh_id', $request->ky_thuat_id);    //  tìm các dự án Trợ giảng
                            $query->orWhere('kt_id', $request->ky_thuat_id);    //  tìm các dự án Giảng viên
                        })
                        ->pluck('bill_id')->toArray();
    ...
    public function update(Request $request)
    {
        try {
            ...
            //  học viên ko được xem bản ghi của người khác
            ...
            //  học viên tự sửa hóa đơn của mình
            ...
            \DB::rollback();
            dd($ex->getMessage());
//            CommonHelper::one_time_message('error', 'Lỗi hệ thống! Vui lòng liên hệ kỹ thuật viên viên.');
...
//            }
            ...
            return response()->json([
                'status' => false,
                'published' => null,
                'msg' => 'Lỗi hệ thống! Vui lòng liên hệ kỹ thuật viên viên.'
            ]);
        }
    }
...
//            }
    ...
    public function searchForSelect2(Request $request)
    {
        ...
        //  học viên ko được xem hóa đơn người khác
        if (CRMEduHelper::getRoleType(\Auth::guard('admin')->user()->id) == 'customer') {
